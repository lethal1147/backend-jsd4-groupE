...
export const postActivities = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "activity card created successfully", rank: nextRank });
  } catch (err) {
    console.log(err);
    res.status(400).send("Creating activity card failed");
  }
};
...
export const getActivity = async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    res.status(400).send("Failed to get activity");
  }
};
...
export const getSingleActivity = async (req, res) => {
  try {
    ...
    console.log(err);
    res.status(400).send("failed to fetch activity");
  }
};
...
export const updateActivity = async (req, res) => {
  ...
  if (req.file) {
    try {
      ...
      console.log(err);
      return res.status(400).send("Updating activity card failed");
    }
  } else {
    try {
      const updatedActivity = {
      ...
      console.log(err);
      return res.status(400).json("Can't update activity card");
    }
  }
};
...
export const deleteActivity = async (req, res) => {
  try {
    ...
    console.log(err);
    res.status(400).send("Deleting activity failed");
  }
};
...
export const countActivity = async (req, res) => {
  try {
    ...
    console.log(err);
    res.status(400).send("Cannot count activity");
  }
};
